#include "stdafx.h"
#include "..\Public\Heart_water_mini.h"
#include "GameInstance.h"
#include <time.h>

CHeart_water_mini::CHeart_water_mini(LPDIRECT3DDEVICE9 pGraphic_Device)
	: CGameObject(pGraphic_Device)
{

}

CHeart_water_mini::CHeart_water_mini(const CHeart_water_mini & rhs)
	: CGameObject(rhs)
{
}

HRESULT CHeart_water_mini::NativeConstruct_Prototype()
{
	if (FAILED(__super::NativeConstruct_Prototype()))
	{
		MSGBOX("__super::NativeConstruct_Prototype returned E_FAIL in CHeart_water_mini::NativeConstruct_Prototype");
		return E_FAIL;
	}
...
HRESULT CHeart_water_mini::NativeConstruct(void * pArg)
{
	if (FAILED(__super::NativeConstruct(pArg)))
	{
		MSGBOX("__super::NativeConstruct returned E_FAIL in CHeart_water_mini::NativeConstruct");
		return E_FAIL;
	}

	if (FAILED(SetUp_Components()))
	{
		MSGBOX("SetUp_Components returned E_FAIL in CHeart_water_mini::NativeConstruct");
		return E_FAIL;
	}
	CGameInstance* pGameInstance = GET_INSTANCE(CGameInstance);
	//srand(time(NULL));

	m_pSlime = (CSlime_Heart*)pArg;
...
_int CHeart_water_mini::Tick(_float fTimeDelta)
{
	if (m_bDead)
		return -1;
...
_int CHeart_water_mini::LateTick(_float fTimeDelta)
{
	if (0 > __super::LateTick(fTimeDelta))
	{
		MSGBOX("0 > __super::LateTick in CHeart_water_mini::LateTick");
		return -1;
	}

	if (nullptr == m_pRendererCom)
	{
		MSGBOX("m_pRendererCom is nullptr in CHeart_water_mini::LateTick");
		return -1;
	}

	if (FAILED(SetUp_OnTerrain()))
	{
		MSGBOX("SetUp_OnTerrain returned E_FAIL in CHeart_water_mini::LateTick");
		return -1;
	}
...
HRESULT CHeart_water_mini::Render()
{
	if (nullptr == m_pVIBufferCom)
	{
		MSGBOX("m_pVIBufferCom is nullptr in CHeart_water_mini::Render")
			return E_FAIL;
	}

	if (FAILED(m_pTransformCom->Bind_OnGraphicDevice()))
	{
		MSGBOX("m_pTransformCom->Bind_OnGraphicDevice returned E_FAIL in CHeart_water_mini::Render");
		return E_FAIL;
	}
	m_pVIBufferCom->Base_Setting();
	if (FAILED(m_pTextureCom->Bind_OnGraphicDevice()))
	{
		MSGBOX("m_pTextureCom->Bind_OnGraphicDevice in CHeart_water_mini::Render");
		return E_FAIL;
	}
	...
	{
		MSGBOX("SetUp_RenderState returned E_FAIL in CHeart_water_mini::Render");
		return E_FAIL;
	}
	...
	{
		MSGBOX("Release_RenderState returned E_FAIL in CHeart_water_mini::Render");
		return E_FAIL;
	}
	////////////////////
...
HRESULT CHeart_water_mini::SetUp_Components()
{
	...
	{
		MSGBOX("__super::Add_Component returned E_FAIL in CHeart_water_mini::SetUp_Components(Transform)");
		return E_FAIL;
	}

	if (FAILED(__super::Add_Component(LEVEL_STATIC, TEXT("Prototype_Component_Renderer"), TEXT("Com_Renderer"), (CComponent**)&m_pRendererCom)))
	{
		MSGBOX("__super::Add_Component returned E_FAIL in CHeart_water_mini::SetUp_Components(Renderer)");
		return E_FAIL;
	}

	if (FAILED(__super::Add_Component(LEVEL_STATIC, TEXT("Prototype_Component_VIBuffer_Rect"), TEXT("Com_VIBuffer"), (CComponent**)&m_pVIBufferCom)))
	{
		MSGBOX("__super::Add_Component returned E_FAIL in CHeart_water_mini::SetUp_Components(VIBuffer_Rect)");
		return E_FAIL;
	}

	if (FAILED(__super::Add_Component(LEVEL_SLIME, TEXT("Prototype_Component_Texture_Heart_mini"), TEXT("Com_Texture"), (CComponent**)&m_pTextureCom)))
	{
		MSGBOX("__super::Add_Component returned E_FAIL in CHeart_water_mini::SetUp_Components(Texture)");
		return E_FAIL;
	}
	...
	{
		MSGBOX("__super::Add_Component returned E_FAIL in CHeart_water_mini::SetUp_Components(VIBuffer_Cube)");
		return E_FAIL;
	}
	//#endif // DEBUG
...
HRESULT CHeart_water_mini::SetUp_OnTerrain()
{
	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);
	...
	if (m_bJump == true) {
		if (m_pTransformCom->Get_State(CTransform::STATE_POSITION).y < vPosition.y) {
			if (FAILED(pGameInstance->Add_GameObject(LEVEL_SLIME, TEXT("Layer_Heart_Water"), TEXT("Prototype_GameObject_Heart_water"), this)))
			{
...
HRESULT CHeart_water_mini::SetUp_RenderState()
{
	if (nullptr == m_pGraphic_Device)
	{
		MSGBOX("m_pGraphic_Device is nullptr in CHeart_water_mini::SetUp_RenderState");
		return E_FAIL;
	}
...
HRESULT CHeart_water_mini::Release_RenderState()
{
	if (nullptr == m_pGraphic_Device)
	{
		MSGBOX("m_pGraphic_Device is nullptr in CHeart_water_mini::Release_RenderState");
		return E_FAIL;
	}
...
CHeart_water_mini * CHeart_water_mini::Create(LPDIRECT3DDEVICE9 pGraphic_Device)
{
	CHeart_water_mini*	pInstance = new CHeart_water_mini(pGraphic_Device);

	if (FAILED(pInstance->NativeConstruct_Prototype()))
	{
		MSGBOX("Failed to Create Slime in CHeart_water_mini::Create");
...
CGameObject * CHeart_water_mini::Clone(void* pArg)
{
	/* 새로운객체를 복제하여 생성한다. */
	CHeart_water_mini*	pInstance = new CHeart_water_mini(*this);


	if (FAILED(pInstance->NativeConstruct(pArg)))
	{
		MSGBOX("Failed to Create Slime in CHeart_water_mini::Clone");
		Safe_Release(pInstance);
	}
	return pInstance;
}

void CHeart_water_mini::Free()
{
	__super::Free();
