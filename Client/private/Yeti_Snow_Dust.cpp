#include "stdafx.h"
#include "..\Public\Yeti_Snow_Dust.h"
#include "GameInstance.h"

CYeti_Snow_Dust::CYeti_Snow_Dust(LPDIRECT3DDEVICE9 pGraphic_Device)
	: CGameObject(pGraphic_Device)
{

}

CYeti_Snow_Dust::CYeti_Snow_Dust(const CYeti_Snow_Dust& rhs)
	: CGameObject(rhs)
{
}

HRESULT CYeti_Snow_Dust::NativeConstruct_Prototype()
{
...
HRESULT CYeti_Snow_Dust::NativeConstruct(void * pArg)
{
...
_int CYeti_Snow_Dust::Tick(_float fTimeDelta)
{
...
_int CYeti_Snow_Dust::LateTick(_float fTimeDelta)
{
	if (0 > __super::LateTick(fTimeDelta))
	{
		MSGBOX("0 > __super::LateTick in CYeti_Snow_Dust::LateTick");
		return -1;
	}

	if (nullptr == m_pRendererCom)
	{
		MSGBOX("m_pRendererCom is nullptr in CYeti_Snow_Dust::LateTick");
		return -1;
	}
...
HRESULT CYeti_Snow_Dust::Render()
{
	if (nullptr == m_pVIBufferCom)
	{
		MSGBOX("m_pVIBufferCom is nullptr in CYeti_Snow_Dust::Render")
			return E_FAIL;
	}

	if (FAILED(m_pTransformCom->Bind_OnGraphicDevice()))
	{
		MSGBOX("m_pTransformCom->Bind_OnGraphicDevice returned E_FAIL in CYeti_Snow_Dust::Render");
		return E_FAIL;
	}
	...
	{
		MSGBOX("m_pTextureCom->Bind_OnGraphicDevice in CYeti_Snow_Dust::Render");
		return E_FAIL;
	}
	...
	{
		MSGBOX("SetUp_RenderState returned E_FAIL in CYeti_Snow_Dust::Render");
		return E_FAIL;
	}
	...
	{
		MSGBOX("Release_RenderState returned E_FAIL in CYeti_Snow_Dust::Render");
		return E_FAIL;
	}
	////////////////////
	return S_OK;
...
HRESULT CYeti_Snow_Dust::Go_Sprites()
{
...
HRESULT CYeti_Snow_Dust::SetUp_Components()
{
	/* For.Com_Transform */
	...
	{
		MSGBOX("__super::Add_Component returned E_FAIL in CYeti_Snow_Dust::SetUp_Components(Transform)");
		return E_FAIL;
	}

	if (FAILED(__super::Add_Component(LEVEL_STATIC, TEXT("Prototype_Component_Renderer"), TEXT("Com_Renderer"), (CComponent**)&m_pRendererCom)))
	{
		MSGBOX("__super::Add_Component returned E_FAIL in CYeti_Snow_Dust::SetUp_Components(Renderer)");
		return E_FAIL;
	}

	if (FAILED(__super::Add_Component(LEVEL_STATIC, TEXT("Prototype_Component_VIBuffer_Rect"), TEXT("Com_VIBuffer"), (CComponent**)&m_pVIBufferCom)))
	{
		MSGBOX("__super::Add_Component returned E_FAIL in CYeti_Snow_Dust::SetUp_Components(VIBuffer_Rect)");
		return E_FAIL;
	}
...
HRESULT CYeti_Snow_Dust::SetUp_OnTerrain()
{
	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);
...
HRESULT CYeti_Snow_Dust::SetUp_RenderState()
{
...
HRESULT CYeti_Snow_Dust::Release_RenderState()
{
	if (nullptr == m_pGraphic_Device)
		return E_FAIL;
...
CYeti_Snow_Dust* CYeti_Snow_Dust::Create(LPDIRECT3DDEVICE9 pGraphic_Device)
{
	CYeti_Snow_Dust*	pInstance = new CYeti_Snow_Dust(pGraphic_Device);

	if (FAILED(pInstance->NativeConstruct_Prototype()))
	{
		MSGBOX("Failed to Creating CYeti_Snow_Dust");
...
CGameObject * CYeti_Snow_Dust::Clone(void* pArg)
{
	/* 새로운객체를 복제하여 생성한다. */
	CYeti_Snow_Dust*	pInstance = new CYeti_Snow_Dust(*this);
