#include "stdafx.h"
#include "..\Public\Yeti_Snow_Break_Effect.h"
#include "GameInstance.h"

CYeti_Snow_Break_Effect::CYeti_Snow_Break_Effect(LPDIRECT3DDEVICE9 pGraphic_Device)
	: CGameObject(pGraphic_Device)
{

}

CYeti_Snow_Break_Effect::CYeti_Snow_Break_Effect(const CYeti_Snow_Break_Effect& rhs)
	: CGameObject(rhs)
{
}

HRESULT CYeti_Snow_Break_Effect::NativeConstruct_Prototype()
{
...
HRESULT CYeti_Snow_Break_Effect::NativeConstruct(void * pArg)
{
	...
	m_tFrame.iFrameStartX = 5.f;
	m_tFrame.iFrameEndX = m_tFrame.iFrameStartX;
	m_tFrame.iFrameCurrentX = m_tFrame.iFrameStartX;
	m_tFrame.iFrameHeight = 80.f;
	m_tFrame.iFrameWidth = 80.f;
	m_tFrame.iTexHeight = 80.f;
	m_tFrame.iTexWidth = 480.f;
	m_fSpriteSpeed = 2;


	m_fScale = ((rand() % 30)) * 0.1f;
...
_int CYeti_Snow_Break_Effect::Tick(_float fTimeDelta)
{
...
_int CYeti_Snow_Break_Effect::LateTick(_float fTimeDelta)
{
	if (0 > __super::LateTick(fTimeDelta))
	{
		MSGBOX("0 > __super::LateTick in CYeti_Snow_Break_Effect::LateTick");
		return -1;
	}

	if (nullptr == m_pRendererCom)
	{
		MSGBOX("m_pRendererCom is nullptr in CYeti_Snow_Break_Effect::LateTick");
		return -1;
	}
...
HRESULT CYeti_Snow_Break_Effect::Render()
{
	if (nullptr == m_pVIBufferCom)
	{
		MSGBOX("m_pVIBufferCom is nullptr in CYeti_Snow_Break_Effect::Render")
			return E_FAIL;
	}

	if (FAILED(m_pTransformCom->Bind_OnGraphicDevice()))
	{
		MSGBOX("m_pTransformCom->Bind_OnGraphicDevice returned E_FAIL in CYeti_Snow_Break_Effect::Render");
		return E_FAIL;
	}
	...
	{
		MSGBOX("m_pTextureCom->Bind_OnGraphicDevice in CYeti_Snow_Break_Effect::Render");
		return E_FAIL;
	}
	...
	{
		MSGBOX("SetUp_RenderState returned E_FAIL in CYeti_Snow_Break_Effect::Render");
		return E_FAIL;
	}
	...
	{
		MSGBOX("Release_RenderState returned E_FAIL in CYeti_Snow_Break_Effect::Render");
		return E_FAIL;
	}
	////////////////////
	return S_OK;
...
HRESULT CYeti_Snow_Break_Effect::Go_Sprites()
{
...
HRESULT CYeti_Snow_Break_Effect::SetUp_Components()
{
	/* For.Com_Transform */
	...
	{
		MSGBOX("__super::Add_Component returned E_FAIL in CYeti_Snow_Break_Effect::SetUp_Components(Transform)");
		return E_FAIL;
	}

	if (FAILED(__super::Add_Component(LEVEL_STATIC, TEXT("Prototype_Component_Renderer"), TEXT("Com_Renderer"), (CComponent**)&m_pRendererCom)))
	{
		MSGBOX("__super::Add_Component returned E_FAIL in CYeti_Snow_Break_Effect::SetUp_Components(Renderer)");
		return E_FAIL;
	}

	if (FAILED(__super::Add_Component(LEVEL_STATIC, TEXT("Prototype_Component_VIBuffer_Rect"), TEXT("Com_VIBuffer"), (CComponent**)&m_pVIBufferCom)))
	{
		MSGBOX("__super::Add_Component returned E_FAIL in CYeti_Snow_Break_Effect::SetUp_Components(VIBuffer_Rect)");
		return E_FAIL;
	}

	/* For.Com_Texture */
	if (FAILED(__super::Add_Component(LEVEL_YETI, TEXT("Prototype_Component_Texture_Yeti_Paticle"), TEXT("Com_Texture"), (CComponent**)&m_pTextureCom)))
		return E_FAIL;
...
HRESULT CYeti_Snow_Break_Effect::SetUp_OnTerrain()
{
	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);
...
HRESULT CYeti_Snow_Break_Effect::SetUp_RenderState()
{
...
HRESULT CYeti_Snow_Break_Effect::Release_RenderState()
{
	if (nullptr == m_pGraphic_Device)
		return E_FAIL;
...
CYeti_Snow_Break_Effect* CYeti_Snow_Break_Effect::Create(LPDIRECT3DDEVICE9 pGraphic_Device)
{
	CYeti_Snow_Break_Effect*	pInstance = new CYeti_Snow_Break_Effect(pGraphic_Device);

	if (FAILED(pInstance->NativeConstruct_Prototype()))
	{
		MSGBOX("Failed to Creating CYeti_Snow_Break_Effect");
...
CGameObject * CYeti_Snow_Break_Effect::Clone(void* pArg)
{
	/* 새로운객체를 복제하여 생성한다. */
	CYeti_Snow_Break_Effect*	pInstance = new CYeti_Snow_Break_Effect(*this);
...
void CYeti_Snow_Break_Effect::Free()
{
	__super::Free();
