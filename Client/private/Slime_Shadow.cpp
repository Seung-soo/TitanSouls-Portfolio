#include "stdafx.h"
#include "..\Public\Slime_Shadow.h"
#include "GameInstance.h"

CSlime_Shadow::CSlime_Shadow(LPDIRECT3DDEVICE9 pGraphic_Device)
	: CGameObject(pGraphic_Device)
{

}

CSlime_Shadow::CSlime_Shadow(const CSlime_Shadow& rhs)
	: CGameObject(rhs)
{
}

HRESULT CSlime_Shadow::NativeConstruct_Prototype()
{
...
HRESULT CSlime_Shadow::NativeConstruct(void * pArg)
{
...
_int CSlime_Shadow::Tick(_float fTimeDelta)
{
...
_int CSlime_Shadow::LateTick(_float fTimeDelta)
{

	if (m_pSlime->Get_Dead() == true) {
	...
	{
		MSGBOX("0 > __super::LateTick in CSlime_Shadow::LateTick");
		return -1;
	}

	if (nullptr == m_pRendererCom)
	{
		MSGBOX("m_pRendererCom is nullptr in CSlime_Shadow::LateTick");
		return -1;
	}

	/*if (FAILED(SetUp_OnTerrain()))
	{
	MSGBOX("SetUp_OnTerrain returned E_FAIL in CSlime_Shadow::LateTick");
	return -1;
	}*/
...
HRESULT CSlime_Shadow::Render()
{
	if (nullptr == m_pVIBufferCom)
	{
		MSGBOX("m_pVIBufferCom is nullptr in CSlime_Shadow::Render")
			return E_FAIL;
	}

	if (FAILED(m_pTransformCom->Bind_OnGraphicDevice()))
	{
		MSGBOX("m_pTransformCom->Bind_OnGraphicDevice returned E_FAIL in CSlime_Shadow::Render");
		return E_FAIL;
	}
	m_pVIBufferCom->Base_Setting();
	if (FAILED(m_pTextureCom->Bind_OnGraphicDevice()))
	{
		MSGBOX("m_pTextureCom->Bind_OnGraphicDevice in CSlime_Shadow::Render");
		return E_FAIL;
	}
	...
	{
		MSGBOX("SetUp_RenderState returned E_FAIL in CSlime_Shadow::Render");
		return E_FAIL;
	}
	...
	{
		MSGBOX("Release_RenderState returned E_FAIL in CSlime_Shadow::Render");
		return E_FAIL;
	}
	////////////////////
	return S_OK;
...
HRESULT CSlime_Shadow::SetUp_Components()
{
	/* For.Com_Transform */
	...
	{
		MSGBOX("__super::Add_Component returned E_FAIL in CSlime_Shadow::SetUp_Components(Transform)");
		return E_FAIL;
	}

	if (FAILED(__super::Add_Component(LEVEL_STATIC, TEXT("Prototype_Component_Renderer"), TEXT("Com_Renderer"), (CComponent**)&m_pRendererCom)))
	{
		MSGBOX("__super::Add_Component returned E_FAIL in CSlime_Shadow::SetUp_Components(Renderer)");
		return E_FAIL;
	}
	...
	{
		MSGBOX("__super::Add_Component returned E_FAIL in CSlime_Shadow::SetUp_Components(VIBuffer_Rect)");
		return E_FAIL;
	}
...
HRESULT CSlime_Shadow::SetUp_OnTerrain()
{
	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);
...
HRESULT CSlime_Shadow::SetUp_RenderState()
{
...
HRESULT CSlime_Shadow::Release_RenderState()
{
	if (nullptr == m_pGraphic_Device)
		return E_FAIL;
...
CSlime_Shadow* CSlime_Shadow::Create(LPDIRECT3DDEVICE9 pGraphic_Device)
{
	CSlime_Shadow*	pInstance = new CSlime_Shadow(pGraphic_Device);

	if (FAILED(pInstance->NativeConstruct_Prototype()))
	{
		MSGBOX("Failed to Creating CSlime_Shadow");
...
CGameObject * CSlime_Shadow::Clone(void* pArg)
{
	/* 새로운객체를 복제하여 생성한다. */
	CSlime_Shadow*	pInstance = new CSlime_Shadow(*this);
