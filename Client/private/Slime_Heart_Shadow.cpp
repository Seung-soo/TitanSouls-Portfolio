#include "stdafx.h"
#include "..\Public\Slime_Heart_Shadow.h"
#include "GameInstance.h"

CSlime_Heart_Shadow::CSlime_Heart_Shadow(LPDIRECT3DDEVICE9 pGraphic_Device)
	: CGameObject(pGraphic_Device)
{

}

CSlime_Heart_Shadow::CSlime_Heart_Shadow(const CSlime_Heart_Shadow& rhs)
	: CGameObject(rhs)
{
}

HRESULT CSlime_Heart_Shadow::NativeConstruct_Prototype()
{
...
HRESULT CSlime_Heart_Shadow::NativeConstruct(void * pArg)
{
	...
	m_pHeart = (CSlime_Heart*)pArg;
	m_fScale = m_pHeart->Get_TransForm()->Get_Scale().x;
...
_int CSlime_Heart_Shadow::Tick(_float fTimeDelta)
{
	...
	_float3 ShadowPos = m_pHeart->Get_TransForm()->Get_State(CTransform::STATE_POSITION);

	//if(ShadowPos.y< 0)
	ShadowPos.y = 0.01f;
	m_fSlimePos = m_pHeart->Get_TransForm()->Get_State(CTransform::STATE_POSITION);
	_float lengh = D3DXVec3Length(&(m_fSlimePos - ShadowPos));
	if (m_pHeart->Get_Jump() == true) {
		lengh = lengh * 0.2f;
...
_int CSlime_Heart_Shadow::LateTick(_float fTimeDelta)
{
	if (0 > __super::LateTick(fTimeDelta))
	{
		MSGBOX("0 > __super::LateTick in CSlime_Heart_Shadow::LateTick");
		return -1;
	}

	if (nullptr == m_pRendererCom)
	{
		MSGBOX("m_pRendererCom is nullptr in CSlime_Heart_Shadow::LateTick");
		return -1;
	}

	/*if (FAILED(SetUp_OnTerrain()))
	{
	MSGBOX("SetUp_OnTerrain returned E_FAIL in CSlime_Heart_Shadow::LateTick");
	return -1;
	}*/
...
HRESULT CSlime_Heart_Shadow::Render()
{
	if (nullptr == m_pVIBufferCom)
	{
		MSGBOX("m_pVIBufferCom is nullptr in CSlime_Heart_Shadow::Render")
			return E_FAIL;
	}

	if (FAILED(m_pTransformCom->Bind_OnGraphicDevice()))
	{
		MSGBOX("m_pTransformCom->Bind_OnGraphicDevice returned E_FAIL in CSlime_Heart_Shadow::Render");
		return E_FAIL;
	}
	m_pVIBufferCom->Base_Setting();
	if (FAILED(m_pTextureCom->Bind_OnGraphicDevice()))
	{
		MSGBOX("m_pTextureCom->Bind_OnGraphicDevice in CSlime_Heart_Shadow::Render");
		return E_FAIL;
	}
	...
	{
		MSGBOX("SetUp_RenderState returned E_FAIL in CSlime_Heart_Shadow::Render");
		return E_FAIL;
	}
	...
	{
		MSGBOX("Release_RenderState returned E_FAIL in CSlime_Heart_Shadow::Render");
		return E_FAIL;
	}
	////////////////////
	return S_OK;
...
HRESULT CSlime_Heart_Shadow::SetUp_Components()
{
	/* For.Com_Transform */
	...
	{
		MSGBOX("__super::Add_Component returned E_FAIL in CSlime_Heart_Shadow::SetUp_Components(Transform)");
		return E_FAIL;
	}

	if (FAILED(__super::Add_Component(LEVEL_STATIC, TEXT("Prototype_Component_Renderer"), TEXT("Com_Renderer"), (CComponent**)&m_pRendererCom)))
	{
		MSGBOX("__super::Add_Component returned E_FAIL in CSlime_Heart_Shadow::SetUp_Components(Renderer)");
		return E_FAIL;
	}
	...
	{
		MSGBOX("__super::Add_Component returned E_FAIL in CSlime_Heart_Shadow::SetUp_Components(VIBuffer_Rect)");
		return E_FAIL;
	}

	/* For.Com_Texture */
	if (FAILED(__super::Add_Component(LEVEL_SLIME, TEXT("Prototype_Component_Texture_Slime_Heart_Shadow"), TEXT("Com_Texture"), (CComponent**)&m_pTextureCom)))
		return E_FAIL;
...
HRESULT CSlime_Heart_Shadow::SetUp_OnTerrain()
{
	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);
...
HRESULT CSlime_Heart_Shadow::SetUp_RenderState()
{
...
HRESULT CSlime_Heart_Shadow::Release_RenderState()
{
	if (nullptr == m_pGraphic_Device)
		return E_FAIL;
...
CSlime_Heart_Shadow* CSlime_Heart_Shadow::Create(LPDIRECT3DDEVICE9 pGraphic_Device)
{
	CSlime_Heart_Shadow*	pInstance = new CSlime_Heart_Shadow(pGraphic_Device);

	if (FAILED(pInstance->NativeConstruct_Prototype()))
	{
		MSGBOX("Failed to Creating CSlime_Heart_Shadow");
...
CGameObject * CSlime_Heart_Shadow::Clone(void* pArg)
{
	/* 새로운객체를 복제하여 생성한다. */
	CSlime_Heart_Shadow*	pInstance = new CSlime_Heart_Shadow(*this);
