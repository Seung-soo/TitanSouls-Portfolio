#include "stdafx.h"
#include "..\Public\Ice_Shadow.h"
#include "GameInstance.h"

CIce_Shadow::CIce_Shadow(LPDIRECT3DDEVICE9 pGraphic_Device)
	: CGameObject(pGraphic_Device)
{

}

CIce_Shadow::CIce_Shadow(const CIce_Shadow& rhs)
	: CGameObject(rhs)
{
}

HRESULT CIce_Shadow::NativeConstruct_Prototype()
{
...
HRESULT CIce_Shadow::NativeConstruct(void * pArg)
{
...
_int CIce_Shadow::Tick(_float fTimeDelta)
{
	...
	_float3 ShadowPos = m_pIce->Get_TransForm()->Get_State(CTransform::STATE_POSITION);

	//if(ShadowPos.y< 0)
	ShadowPos.y = 0.01f;
	m_fFistPos = m_pIce->Get_TransForm()->Get_State(CTransform::STATE_POSITION);
	_float lengh = D3DXVec3Length(&(m_fFistPos - ShadowPos));
	if (m_pIce->Get_Jump() == true) {
		lengh = lengh * 0.2f;
	...
	if (m_pIce->Get_Search()) {
		m_pTransformCom->Set_State(CTransform::STATE_RIGHT, m_pIce->Get_TransForm()->Get_State(CTransform::STATE_RIGHT));
		m_pTransformCom->Set_State(CTransform::STATE_UP, m_pIce->Get_TransForm()->Get_State(CTransform::STATE_UP));
		m_pTransformCom->Set_State(CTransform::STATE_LOOK, m_pIce->Get_TransForm()->Get_State(CTransform::STATE_LOOK));
	}
	m_pTransformCom->Set_State(CTransform::STATE_POSITION, ShadowPos);

	if (m_pIce->Get_Hir()) {
		m_pTransformCom->Scaled(_float3(m_pIce->Get_Scale(), m_pIce->Get_Scale(), m_pIce->Get_Scale()));
	}
...
_int CIce_Shadow::LateTick(_float fTimeDelta)
{
	if (m_pIce->Get_Dead() == true) {
	...
	{
		MSGBOX("0 > __super::LateTick in CIce_Shadow::LateTick");
		return -1;
	}

	if (nullptr == m_pRendererCom)
	{
		MSGBOX("m_pRendererCom is nullptr in CIce_Shadow::LateTick");
		return -1;
	}
...
HRESULT CIce_Shadow::Render()
{
	if (nullptr == m_pVIBufferCom)
	{
		MSGBOX("m_pVIBufferCom is nullptr in CIce_Shadow::Render")
			return E_FAIL;
	}

	if (FAILED(m_pTransformCom->Bind_OnGraphicDevice()))
	{
		MSGBOX("m_pTransformCom->Bind_OnGraphicDevice returned E_FAIL in CIce_Shadow::Render");
		return E_FAIL;
	}
	m_pVIBufferCom->Base_Setting();
	if (FAILED(m_pTextureCom->Bind_OnGraphicDevice()))
	{
		MSGBOX("m_pTextureCom->Bind_OnGraphicDevice in CIce_Shadow::Render");
		return E_FAIL;
	}
	...
	{
		MSGBOX("SetUp_RenderState returned E_FAIL in CIce_Shadow::Render");
		return E_FAIL;
	}
	...
	{
		MSGBOX("Release_RenderState returned E_FAIL in CIce_Shadow::Render");
		return E_FAIL;
	}
	////////////////////
	return S_OK;
...
HRESULT CIce_Shadow::SetUp_Components()
{
	/* For.Com_Transform */
	...
	{
		MSGBOX("__super::Add_Component returned E_FAIL in CIce_Shadow::SetUp_Components(Transform)");
		return E_FAIL;
	}

	if (FAILED(__super::Add_Component(LEVEL_STATIC, TEXT("Prototype_Component_Renderer"), TEXT("Com_Renderer"), (CComponent**)&m_pRendererCom)))
	{
		MSGBOX("__super::Add_Component returned E_FAIL in CIce_Shadow::SetUp_Components(Renderer)");
		return E_FAIL;
	}
	...
	{
		MSGBOX("__super::Add_Component returned E_FAIL in CIce_Shadow::SetUp_Components(VIBuffer_Rect)");
		return E_FAIL;
	}

	/* For.Com_Texture */
	if (FAILED(__super::Add_Component(LEVEL_ICEBRAIN, TEXT("Prototype_Component_Texture_Ice_Shadow"), TEXT("Com_Texture"), (CComponent**)&m_pTextureCom)))
		return E_FAIL;
...
HRESULT CIce_Shadow::SetUp_OnTerrain()
{
	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);
...
HRESULT CIce_Shadow::SetUp_RenderState()
{
...
HRESULT CIce_Shadow::Release_RenderState()
{
	if (nullptr == m_pGraphic_Device)
		return E_FAIL;
...
CIce_Shadow* CIce_Shadow::Create(LPDIRECT3DDEVICE9 pGraphic_Device)
{
	CIce_Shadow*	pInstance = new CIce_Shadow(pGraphic_Device);

	if (FAILED(pInstance->NativeConstruct_Prototype()))
	{
		MSGBOX("Failed to Creating CIce_Shadow");
...
CGameObject * CIce_Shadow::Clone(void* pArg)
{
	/* 새로운객체를 복제하여 생성한다. */
	CIce_Shadow*	pInstance = new CIce_Shadow(*this);
