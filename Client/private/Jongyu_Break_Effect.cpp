#include "stdafx.h"
#include "..\Public\Jongyu_Break_Effect.h"
#include "GameInstance.h"
#include <time.h>

CJongyu_Break_Effect::CJongyu_Break_Effect(LPDIRECT3DDEVICE9 pGraphic_Device)
	: CGameObject(pGraphic_Device)
{

}

CJongyu_Break_Effect::CJongyu_Break_Effect(const CJongyu_Break_Effect & rhs)
	: CGameObject(rhs)
{
}

HRESULT CJongyu_Break_Effect::NativeConstruct_Prototype()
{
	if (FAILED(__super::NativeConstruct_Prototype()))
	{
		MSGBOX("__super::NativeConstruct_Prototype returned E_FAIL in CJongyu_Break_Effect::NativeConstruct_Prototype");
		return E_FAIL;
	}
...
HRESULT CJongyu_Break_Effect::NativeConstruct(void * pArg)
{
	if (FAILED(__super::NativeConstruct(pArg)))
	{
		MSGBOX("__super::NativeConstruct returned E_FAIL in CJongyu_Break_Effect::NativeConstruct");
		return E_FAIL;
	}

	if (FAILED(SetUp_Components()))
	{
		MSGBOX("SetUp_Components returned E_FAIL in CJongyu_Break_Effect::NativeConstruct");
		return E_FAIL;
	}
	CGameInstance* pGameInstance = GET_INSTANCE(CGameInstance);
	//srand(time(NULL));

	m_pSlime = (CJongyu*)pArg;
	CTransform* a = m_pSlime->Get_TransForm();
	_float3 d = a->Get_State(CTransform::STATE_POSITION);
	d.y = 1.f;
	m_pTransformCom->Set_State(CTransform::STATE_POSITION, d);
	...
	m_tFrame.iFrameStartX = rand() % 3+1;
...
_int CJongyu_Break_Effect::Tick(_float fTimeDelta)
{
	if (m_bDead)
		return -1;
...
_int CJongyu_Break_Effect::LateTick(_float fTimeDelta)
{
	if (0 > __super::LateTick(fTimeDelta))
	{
		MSGBOX("0 > __super::LateTick in CJongyu_Break_Effect::LateTick");
		return -1;
	}

	if (nullptr == m_pRendererCom)
	{
		MSGBOX("m_pRendererCom is nullptr in CJongyu_Break_Effect::LateTick");
		return -1;
	}
...
HRESULT CJongyu_Break_Effect::Render()
{
	if (nullptr == m_pVIBufferCom)
	{
		MSGBOX("m_pVIBufferCom is nullptr in CJongyu_Break_Effect::Render")
			return E_FAIL;
	}

	if (FAILED(m_pTransformCom->Bind_OnGraphicDevice()))
	{
		MSGBOX("m_pTransformCom->Bind_OnGraphicDevice returned E_FAIL in CJongyu_Break_Effect::Render");
		return E_FAIL;
	}
	...
	{
		MSGBOX("m_pTextureCom->Bind_OnGraphicDevice in CJongyu_Break_Effect::Render");
		return E_FAIL;
	}
	...
	{
		MSGBOX("SetUp_RenderState returned E_FAIL in CJongyu_Break_Effect::Render");
		return E_FAIL;
	}
	...
	{
		MSGBOX("Release_RenderState returned E_FAIL in CJongyu_Break_Effect::Render");
		return E_FAIL;
	}
	////////////////////
...
HRESULT CJongyu_Break_Effect::SetUp_Components()
{
	...
	{
		MSGBOX("__super::Add_Component returned E_FAIL in CJongyu_Break_Effect::SetUp_Components(Transform)");
		return E_FAIL;
	}

	if (FAILED(__super::Add_Component(LEVEL_STATIC, TEXT("Prototype_Component_Renderer"), TEXT("Com_Renderer"), (CComponent**)&m_pRendererCom)))
	{
		MSGBOX("__super::Add_Component returned E_FAIL in CJongyu_Break_Effect::SetUp_Components(Renderer)");
		return E_FAIL;
	}

	if (FAILED(__super::Add_Component(LEVEL_STATIC, TEXT("Prototype_Component_VIBuffer_Rect"), TEXT("Com_VIBuffer"), (CComponent**)&m_pVIBufferCom)))
	{
		MSGBOX("__super::Add_Component returned E_FAIL in CJongyu_Break_Effect::SetUp_Components(VIBuffer_Rect)");
		return E_FAIL;
	}

	if (FAILED(__super::Add_Component(LEVEL_COLOSUSS, TEXT("Prototype_Component_Texture_Jongyu_Break"), TEXT("Com_Texture"), (CComponent**)&m_pTextureCom)))
	{
		MSGBOX("__super::Add_Component returned E_FAIL in CJongyu_Break_Effect::SetUp_Components(Texture)");
		return E_FAIL;
	}
	...
	{
		MSGBOX("__super::Add_Component returned E_FAIL in CJongyu_Break_Effect::SetUp_Components(VIBuffer_Cube)");
		return E_FAIL;
	}
	//#endif // DEBUG
...
HRESULT CJongyu_Break_Effect::SetUp_OnTerrain()
{
	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);
...
HRESULT CJongyu_Break_Effect::SetUp_RenderState()
{
	if (nullptr == m_pGraphic_Device)
	{
		MSGBOX("m_pGraphic_Device is nullptr in CJongyu_Break_Effect::SetUp_RenderState");
		return E_FAIL;
	}
...
HRESULT CJongyu_Break_Effect::Release_RenderState()
{
	if (nullptr == m_pGraphic_Device)
	{
		MSGBOX("m_pGraphic_Device is nullptr in CJongyu_Break_Effect::Release_RenderState");
		return E_FAIL;
	}
...
HRESULT CJongyu_Break_Effect::Go_Sprites()
{
...
CJongyu_Break_Effect * CJongyu_Break_Effect::Create(LPDIRECT3DDEVICE9 pGraphic_Device)
{
	CJongyu_Break_Effect*	pInstance = new CJongyu_Break_Effect(pGraphic_Device);

	if (FAILED(pInstance->NativeConstruct_Prototype()))
	{
		MSGBOX("Failed to Create Slime in CJongyu_Break_Effect::Create");
...
CGameObject * CJongyu_Break_Effect::Clone(void* pArg)
{
	/* 새로운객체를 복제하여 생성한다. */
	CJongyu_Break_Effect*	pInstance = new CJongyu_Break_Effect(*this);


	if (FAILED(pInstance->NativeConstruct(pArg)))
	{
		MSGBOX("Failed to Create Slime in CJongyu_Break_Effect::Clone");
		Safe_Release(pInstance);
	}
	return pInstance;
}

void CJongyu_Break_Effect::Free()
{
	__super::Free();
