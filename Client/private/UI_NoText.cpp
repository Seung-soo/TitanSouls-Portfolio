#include "stdafx.h"
#include "..\public\UI_NoText.h"
#include "GameInstance.h"

CUI_NoText::CUI_NoText(LPDIRECT3DDEVICE9 pGraphic_Device)
	:CUI(pGraphic_Device)
{
}

CUI_NoText::CUI_NoText(const CUI_NoText & rhs)
	: CUI(rhs)
{
}

HRESULT CUI_NoText::NativeConstruct_Prototype()
{
	//m_iUIDepth = 3;
...
HRESULT CUI_NoText::NativeConstruct(void * pArg)
{
	//m_iUIDepth = 3;
...
_int CUI_NoText::Tick(_float fTimeDelta)
{
	//m_iUIDepth = 3;
...
_int CUI_NoText::LateTick(_float fTimeDelta)
{
	return __super::LateTick(fTimeDelta);
}

HRESULT CUI_NoText::Render()
{
...
HRESULT CUI_NoText::SetUp_Components()
{
...
HRESULT CUI_NoText::SetUp_RenderState()
{
	/*
...
HRESULT CUI_NoText::Go_Sprites(void)
{
	if (FAILED(m_pVIBufferCom->Go_Sprites(m_tFrame))) {
		MSGBOX("m_pVIBufferCom->Go_Sprites(m_tFrame) in CUI_NoText");
		return E_FAIL;
	}
	return S_OK;
}

CUI_NoText * CUI_NoText::Create(LPDIRECT3DDEVICE9 pGraphic_Device)
{
	CUI_NoText*	pInstance = new CUI_NoText(pGraphic_Device);
...
CGameObject * CUI_NoText::Clone(void * pArg)
{
	CGameObject*	pInstance = new CUI_NoText(*this);
