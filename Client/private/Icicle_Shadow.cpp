#include "stdafx.h"
#include "..\Public\Icicle_Shadow.h"
#include "GameInstance.h"

CIcicle_Shadow::CIcicle_Shadow(LPDIRECT3DDEVICE9 pGraphic_Device)
	: CGameObject(pGraphic_Device)
{

}

CIcicle_Shadow::CIcicle_Shadow(const CIcicle_Shadow & rhs)
	: CGameObject(rhs)
{
}

HRESULT CIcicle_Shadow::NativeConstruct_Prototype()
{
...
HRESULT CIcicle_Shadow::NativeConstruct(void * pArg)
{
...
_int CIcicle_Shadow::Tick(_float fTimeDelta)
{
...
_int CIcicle_Shadow::LateTick(_float fTimeDelta)
{
	...
	{
		MSGBOX("0 > __super::LateTick in CIcicle_Shadow::LateTick");
		return -1;
	}

	if (nullptr == m_pRendererCom)
	{
		MSGBOX("m_pRendererCom is nullptr in CIcicle_Shadow::LateTick");
		return -1;
	}

	/*if (FAILED(SetUp_OnTerrain()))
	{
		MSGBOX("SetUp_OnTerrain returned E_FAIL in CIcicle_Shadow::LateTick");
		return -1;
	}*/
...
HRESULT CIcicle_Shadow::Render()
{
	if (nullptr == m_pVIBufferCom)
	{
		MSGBOX("m_pVIBufferCom is nullptr in CIcicle_Shadow::Render")
			return E_FAIL;
	}

	if (FAILED(m_pTransformCom->Bind_OnGraphicDevice()))
	{
		MSGBOX("m_pTransformCom->Bind_OnGraphicDevice returned E_FAIL in CIcicle_Shadow::Render");
		return E_FAIL;
	}
	m_pVIBufferCom->Base_Setting();
	if (FAILED(m_pTextureCom->Bind_OnGraphicDevice()))
	{
		MSGBOX("m_pTextureCom->Bind_OnGraphicDevice in CIcicle_Shadow::Render");
		return E_FAIL;
	}
	...
	{
		MSGBOX("SetUp_RenderState returned E_FAIL in CIcicle_Shadow::Render");
		return E_FAIL;
	}
...
HRESULT CIcicle_Shadow::SetUp_Components()
{
	/* For.Com_Transform */
	...
	{
		MSGBOX("__super::Add_Component returned E_FAIL in CIcicle_Shadow::SetUp_Components(Transform)");
		return E_FAIL;
	}

	if (FAILED(__super::Add_Component(LEVEL_STATIC, TEXT("Prototype_Component_Renderer"), TEXT("Com_Renderer"), (CComponent**)&m_pRendererCom)))
	{
		MSGBOX("__super::Add_Component returned E_FAIL in CIcicle_Shadow::SetUp_Components(Renderer)");
		return E_FAIL;
	}

	if (FAILED(__super::Add_Component(LEVEL_STATIC, TEXT("Prototype_Component_VIBuffer_Rect"), TEXT("Com_VIBuffer"), (CComponent**)&m_pVIBufferCom)))
	{
		MSGBOX("__super::Add_Component returned E_FAIL in CIcicle_Shadow::SetUp_Components(VIBuffer_Rect)");
		return E_FAIL;
	}
...
HRESULT CIcicle_Shadow::SetUp_OnTerrain()
{
	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);
...
HRESULT CIcicle_Shadow::SetUp_RenderState()
{
...
HRESULT CIcicle_Shadow::Release_RenderState()
{
	if (nullptr == m_pGraphic_Device)
		return E_FAIL;
...
CIcicle_Shadow * CIcicle_Shadow::Create(LPDIRECT3DDEVICE9 pGraphic_Device)
{
	CIcicle_Shadow*	pInstance = new CIcicle_Shadow(pGraphic_Device);

	if (FAILED(pInstance->NativeConstruct_Prototype()))
	{
		MSGBOX("Failed to Creating CIcicle_Shadow");
...
CGameObject * CIcicle_Shadow::Clone(void* pArg)
{
	/* 새로운객체를 복제하여 생성한다. */
	CIcicle_Shadow*	pInstance = new CIcicle_Shadow(*this);
